...
const exploreVertexFunc =
  (caves) =>
  (vertexName, currentPath, visited, hasVisitedSmallCaveTwice = false) => {
    const { edges } = caves[vertexName];

    // we have now visited a small cave twice - this one
    ...
    for (let edge of edges) {
      if (edge === END_VERTEX) {
        ...
        !isSmallCave(edge) ||
        !visited[edge] ||
        (!hasVisitedSmallCaveTwice && edge !== STARTING_VERTEX)
      ) {
        caves.explore(
          edge,
          [...currentPath, edge],
